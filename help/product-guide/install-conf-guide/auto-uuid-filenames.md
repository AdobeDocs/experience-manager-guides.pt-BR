...
As guias a seguir fornecem instruções para configurar nomes de arquivo automáticos com base na UUID de acordo com a configuração do Experience Manager Guides: Cloud Service ou No local.
...
Use as instruções fornecidas em [Substituições de configuração](download-install-config-override.md#) para criar o arquivo de configuração. No arquivo de configuração, forneça os seguintes detalhes \(propriedade\) para configurar nomes de arquivo automáticos com base na UUID:

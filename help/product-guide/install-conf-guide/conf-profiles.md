...
- **Configurações do Editor XML**: use esta guia para personalizar a aparência e os vários recursos do Editor da Web. As seguintes configurações configuráveis estão disponíveis para o Editor da Web:
   ...
   - Configuração do editor XML somente para Cloud Service
...
Se você estiver usando um atributo personalizado, ele deverá ser um atributo DITA válido suportado pelo DTD. As guias a seguir fornecem instruções para usar qualquer atributo, que não seja um atributo DITA padrão com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
As guias a seguir fornecem instruções para configurar os modelos de criação com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
1. Na página do perfil, clique na guia **Modelo de Criação**.
1. Clique em **Editar**.
...
1. Siga um destes procedimentos para criar ou editar uma predefinição de saída:
   ...
   - Clique em **Editar** para abrir a configuração da predefinição selecionada para edição.
     ...
     Para obter informações sobre as configurações de predefinição de saída, dependendo da configuração usada, consulte *Noções básicas sobre as predefinições de saída* em Uso do Adobe Experience Manager Guides.
...
As guias a seguir fornecem informações com base na configuração do Experience Manager Guides: Cloud Service ou No local.

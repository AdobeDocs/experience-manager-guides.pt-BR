...
### Converter documentos existentes do Word em documento do tipo de tópico DITA

As guias a seguir fornecem instruções para converter documentos existentes do Word em documento do tipo de tópico DITA com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
1. Faça upload dos documentos de origem do Word para esta pasta. Para obter informações sobre como carregar conteúdo no DAM, consulte [Carregar conteúdo DITA existente](upload-dita-content.md#).
...
De maneira geral, o processo de conversão requer que você [Prepare arquivos do InDesign para conversão](aemg-appendix.md#id195DBF0045Z) e [Prepare o arquivo de mapeamento para migração do InDesign para o DITA](aemg-appendix.md#id194AF0003HT). Em seguida, você precisa seguir o procedimento especificado para executar o processo de conversão.
...
1. Crie um nó de sobreposição da pasta `config` no nó `apps`.
   ...
   - No elemento `mapStyle`, especifique o local do arquivo de mapa que contém mapeamentos para estilos de documento do InDesign para elementos DITA. O mapeamento padrão é armazenado no arquivo localizado em:
     ...
     >[!NOTE]
     >
     > Para obter mais informações sobre a estrutura do arquivo `stmap.adobeidml.xml` e como personalizá-lo, consulte [Apêndice](aemg-appendix.md#id194AF0003HT).
...
1. Faça upload dos documentos de origem do InDesign para esta pasta. Para obter informações sobre como carregar conteúdo no DAM, consulte [Carregar conteúdo DITA existente](upload-dita-content.md#).
...
### Converter seu documento XHTML existente em um tópico DITA

As guias a seguir fornecem instruções para converter seu documento XHTML existente em um tópico DITA com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
1. Faça upload dos documentos XHTML de origem para esta pasta. Para obter informações sobre como carregar conteúdo no DAM, consulte [Carregar conteúdo DITA existente](upload-dita-content.md#).
...
Após a conversão, o conteúdo estruturado pode ser migrado para o AEM Guides. Para obter mais detalhes, exiba [Carregar conteúdo DITA existente usando a ferramenta WebDAV e o FrameMaker](./upload-dita-content.md).
...
As guias a seguir fornecem instruções para migrar qualquer outro documento estruturado com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
1. Faça upload dos documentos estruturados de origem nesta pasta. Para obter informações sobre como carregar conteúdo no DAM, consulte [Carregar conteúdo DITA existente](upload-dita-content.md#).

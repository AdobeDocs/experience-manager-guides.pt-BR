...
As guias a seguir fornecem instruções para adicionar elementos ou atributos DITA personalizados no arquivo de serialização de pesquisa com base na configuração do Experience Manager Guides: Cloud Service ou No local.
...
As guias a seguir fornecem instruções para habilitar a opção de extração de metadados no `configMgr` com base na configuração do Experience Manager Guides: Cloud Service ou No Local.

>[!BEGINTABS]

>[!TAB Cloud Service]

Use as instruções fornecidas em [Substituições de configuração](download-install-config-override.md#) para criar o arquivo de configuração. No arquivo de configuração, forneça os seguintes detalhes \(property\) para configurar a opção de extração de metadados:
...
As guias a seguir fornecem instruções para excluir a pasta de tradução temporária da pesquisa com base na configuração do Experience Manager Guides: Cloud Service ou No local.

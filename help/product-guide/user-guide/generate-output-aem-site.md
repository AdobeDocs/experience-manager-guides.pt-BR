...
- Fluxo de trabalho de geração do Post
...
>[!TIP]
>
> Consulte a seção *publicação do site AEM* no guia de Práticas recomendadas para obter as práticas recomendadas sobre como criar a saída do site AEM.
...
| Nome de configuração | Dê um nome descritivo para as configurações do site de AEM que você está criando. Por exemplo, você pode especificar *saída de clientes internos* ou *saída de usuários finais*. |
...
1. Selecione a predefinição de saída **Site AEM**.

   Você obtém as guias Configurações de predefinições de AEM e Contexto do Publish.

   ![](images/aem-site-publish-settings.png){width="800" align="left"}

1. Abra a guia **Publish Context**.
   ...
   >[!NOTE]
   >
   > A guia Contexto do Publish mostra tópicos que são vinculados usando somente o `peer` `@scope`. Para links com `local` `@scope`, não é necessário especificar o contexto de publicação.
...
Para obter mais detalhes, consulte a seção *Configurar nivelamento da estrutura do nó do site AEM* no guia Instalar e configurar o Adobe Experience Manager Guides.

...
description: Saiba como criar uma predefinição de EPUB no painel do mapa. Configurar a predefinição de saída de EPUB nos Guias AEM.
...
| Nome da configuração | Dê um nome descritivo para as configurações de saída de EPUB que você está criando. Por exemplo, você pode especificar _Saída interna de clientes_ ou _saída de usuários finais_. |
...
| Nome do arquivo | Especifique o nome do arquivo com o qual deseja salvar a saída de EPUB.<br><br>**Nota**: Se você não fornecer um nome de arquivo, o título do mapa DITA será usado para gerar o nome final do arquivo de saída do EPUB. Se o mapa não tiver um título, o nome do arquivo do mapa DITA será usado para nomear a saída final do EPUB. O nome do arquivo é limpo usando as regras configuradas no sistema para lidar com qualquer caractere inválido. |

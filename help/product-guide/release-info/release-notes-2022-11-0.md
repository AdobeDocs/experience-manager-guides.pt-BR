...
1. Atualize a propriedade `<dox.version>` no arquivo `/dox/dox.installer/pom.xml` do seu código Git Cloud Service para 2022.11.198.
...
Esta seção lista a matriz de compatibilidade dos aplicativos de software compatíveis com a versão de as a Cloud Service de novembro de 2022 da AEM Guides.
...
* Ao usar uma exportação de PDF nativa, os `<indexterm>` aninhados não são aninhados no índice. (10521)
...
* O processamento do Post está bloqueado para todas as pastas de idioma cujos ativos estão presentes em um projeto de tradução ativo. (10332)

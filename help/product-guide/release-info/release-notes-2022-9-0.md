...
Selecione o ícone **Substituir** para substituir a ocorrência atual e o ícone **Substituir tudo no arquivo** para substituir todas as ocorrências no arquivo selecionado.

![Localizar Substituição no mapa](assets/map-find-replace.png)

Por padrão, as opções **Fazer check-out do arquivo antes de substituir** e **Criar nova versão após substituir** são selecionadas, portanto, um arquivo é submetido a check-out antes da substituição do texto e uma nova versão é criada depois da substituição do texto.

### Visualizar diferença de versão para arquivos Fora de sincronização no painel de tradução

Agora você pode optar por traduzir os arquivos **Fora de sincronização** com base nas alterações feitas entre as duas versões de um tópico.\

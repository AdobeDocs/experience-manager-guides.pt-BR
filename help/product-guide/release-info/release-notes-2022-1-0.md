...
description: Versão de janeiro do  [!DNL Adobe Experience Manager Guides] as a Cloud Service
...
1. Atualize a propriedade `<dox.version>` no arquivo `/dox/dox.installer/pom.xml` do seu código Git do Cloud Services para 2022.1.78.
1. Confirme as alterações e execute o pipeline de Serviços em Nuvem para atualizar para a versão de janeiro do as a Cloud Service [!DNL AEM Guides].

## Matriz de compatibilidade

Esta seção lista a matriz de compatibilidade dos aplicativos de software compatíveis com a versão de janeiro de 2022 do [!DNL AEM Guides] as a Cloud Service.
...
Para criar com o FrameMaker no as a Cloud Service [!DNL AEM Guides], use o FrameMaker versão 2020.4 e superior.
...
* Uma nova configuração foi adicionada para permitir que os usuários retenham ou excluam as versões dos tópicos que estão sendo excluídos quando a opção **Excluir e Criar** é usada na geração de saída do Site do AEM.
...
* Sempre que o usuário cria um arquivo DITA, o nome de arquivo padrão aparece em letras minúsculas para estar em linha com o cenário de criação de pastas do AEM nativo.

### Novo recurso de exportação de relatório

Os relatórios são muito úteis para identificar a integridade do seu conteúdo. O as a Cloud Service [!DNL AEM Guides] fornece vários relatórios para controlar seu conteúdo. Agora, você pode não apenas visualizar os relatórios, mas também exportar os dados do relatório em um arquivo CSV para visualizar e compartilhar com sua equipe maior. Os dados do relatório podem fornecer uma rápida visão de todos os links quebrados ou imagens ausentes.
...
* A publicação falha com uma exceção se a linha de base usada na publicação do site do AEM for excluída. (8572)
...
A Adobe identificou os seguintes problemas conhecidos para a versão de janeiro de 2022 do as a Cloud Service [!DNL AEM Guides].
